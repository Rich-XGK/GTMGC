"""
    Collator for GPS
"""
import torch
from typing import Literal
from typing import Any, Dict, Sequence
from torch_geometric.data import Data, Batch
from torch_geometric.transforms import AddRandomWalkPE, AddLaplacianEigenvectorPE


class GPSCollator:
    def __init__(self, pe_type: Literal["random_walk", "laplacian"] = "random_walk", random_walk_length: int = 20, laplacian_k: int = 10) -> None:
        if pe_type == "random_walk":
            self.pe_f = AddRandomWalkPE(walk_length=random_walk_length, attr_name="pe")
        elif pe_type == "laplacian":
            self.pe_f = AddLaplacianEigenvectorPE(k=laplacian_k, attr_name="pe")
        else:
            raise ValueError(f"{pe_type} has not implemented in GPSCollator yet!")

    @torch.no_grad()
    def __call__(self, mol_sq: Sequence[Dict]) -> Any:
        mol_data_seq = [self._add_pe(self._get_pyg_data(mol)) for mol in mol_sq]
        batch = Batch().from_data_list(mol_data_seq)
        return {
            "node_type": batch.node_type,
            "edge_index": batch.edge_index,
            "node_attr": batch.node_attr,
            "edge_attr": batch.edge_attr,
            "conformer": batch.conformer,
            "batch": batch.batch,
            "pe": batch.pe,
        }

    def _get_pyg_data(self, mol: Dict) -> Data:
        return Data(
            node_type=torch.tensor(mol["node_type"], dtype=torch.long),
            edge_index=torch.tensor(mol["edge_index"], dtype=torch.long),
            node_attr=torch.tensor(mol["node_attr"], dtype=torch.long),
            edge_attr=torch.tensor(mol["edge_attr"], dtype=torch.long),
            conformer=torch.tensor(mol["conformer"], dtype=torch.float32),
        )

    def _add_pe(self, data: Data) -> Data:
        return self.pe_f(data)
